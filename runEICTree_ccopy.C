...
void runEICTree_ccopy(const TString filename="/Users/carorg/projects/eic/eic-centrality/root_files/root-files-2020/eXe_490GeV_fixed_target_40k", const int nEvents = 40000){
    ...
    for(int i(0); i < nEvents; ++i ) {
        ...
        for(int j(0); j < nParticles; ++j ) {
            ...
            //Beta_2 = sqrt(p_gamma_beagle.Px()*p_gamma_beagle.Px()+p_gamma_beagle.Py()*p_gamma_beagle.Py()+p_gamma_beagle.Pz()*p_gamma_beagle.Pz())/(p_gamma_beagle.E()+MASS_PROTON);
            Beta = sqrt(p_gamma_beagle.E()*p_gamma_beagle.E()+trueQ2)/(p_gamma_beagle.E()+MASS_PROTON);
